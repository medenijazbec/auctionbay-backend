...
builder.Services.AddCors(options =>
{
    ...
    {
        policy.WithOrigins(
            "http://192.168.1.13:8080",   // the API itself (same-origin XHR)
            "http://192.168.1.13:5173",   // dev server on the NAS (if any)
            "http://localhost:5173",      // dev server on the same machine
            "http://192.168.1.4:5173"     // dev server running on your PC
        )
          .AllowAnyHeader()
          .AllowAnyMethod()
...
app.Use(async (context, next) =>
{
    ...
    headers["Content-Security-Policy"] =
        ...
        "img-src 'self' data:; " +
        "connect-src 'self' http://192.168.1.13:8080 http://localhost:5173 ws://localhost:5173"
        +" http://192.168.1.4:5173 ws://192.168.1.4:5173;" +
